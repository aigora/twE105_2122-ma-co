    ...
    switch (stage)
    {
        case 2://JUGAR
                while(stage==2){
                    ...
                    SDL_Texture* texture = SDL_CreateTextureFromSurface(window.renderer, surface); //AÑADE LA NUEVA IMAGEN
                    SDL_FreeSurface(surface);
                    SDL_RenderCopy(window.renderer, texture, NULL, NULL);//Genera un efecto de carga
                    SDL_RenderPresent(window.renderer);
                    SDL_Delay(2000);
                    SDL_DestroyTexture(texture);
                    return 2;
